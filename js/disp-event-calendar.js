...
calendar.on( 'select', function( start, end, jsEvent, view ){
	...
	if (view.type == 'agendaWeek') {
		if (allDay) {
			dateEndAllDay = end.subtract( 1, 'days' ).format( 'MM/DD/YYYY' );
		} 
		else {
			dateEndAllDay = end.format( 'MM/DD/YYYY' );
		}
	}
	else {
		dateEndAllDay = end.subtract( 1, 'days' ).format( 'MM/DD/YYYY' );
	...
	openModal( 'Add Booking', './modal-event-add.php?Date_Start=' + dateStart + '&Date_End=' + dateEnd + '&Date_Start_All_Day=' + dateStartAllDay + '&Date_End_All_Day=' + dateEndAllDay );
});

// Open existing Event
calendar.on( 'eventClick', function( event, jsEvent, view ){
	openModal( 'Edit Booking', './modal-event-edit.php?Event_ID=' + event.id );
});
